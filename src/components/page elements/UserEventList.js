...
export default function UserEventList() {
    const EventNumber = useContext(eventNumberContext);
    const currentUser = useContext(loginContext);
    const contextEvents = useContext(eventContext);
    const [currentEventNumber, setEventNumber] = useState();
    const [eventList, setEventList] = useState();
    useEffect(() => {
        setEventList(contextEvents);
    }, [contextEvents])
    ...
    function generateUserEvents(e){
        e.preventDefault();
        let match = eventList.filter(event => event.username == "guest");
        console.log(match);
        setEventList(match);
        // let url = 'http://localhost:9000/user1/username1/guest';
        // axios.get(url).then(
        //     response => { 
        ...
        return (
            <div> 
                {(e) => generateUserEvents(e)}
                <ul class="eventwrapper">
                {
                    /*eventList.map(() => */ <div class = "eventlisting">
                        <ul class="eventinfo">
                        <li>Event: {'name'}</li>
                        <li>Host: {'username'}</li>
                        <li>Sport: {'sport'}</li>
                        ...
                        <li><button class="btn" id="delete-event-button" onClick={() => DeleteEventButton()}>Delete Event</button></li>    
                        </ul>
